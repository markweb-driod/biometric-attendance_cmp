...
@section('page-title', 'Manage Venues')
@section('page-description', 'Create and manage lecture venues with geo-fencing')
...
<div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-6" id="stats-cards">
        <div class="bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl shadow-lg p-5 flex flex-col items-center justify-center text-white relative overflow-hidden">
            ...
            <div class="text-3xl font-extrabold mb-1 z-10">{{ $venues->count() }}</div>
        ...
        <div class="bg-gradient-to-br from-green-400 to-green-600 rounded-xl shadow-lg p-5 flex flex-col items-center justify-center text-white relative overflow-hidden">
            ...
            <div class="text-3xl font-extrabold mb-1 z-10">{{ $venues->where('is_active', true)->count() }}</div>
        ...
        <div class="bg-gradient-to-br from-purple-400 to-purple-600 rounded-xl shadow-lg p-5 flex flex-col items-center justify-center text-white relative overflow-hidden">
            ...
            <div class="text-3xl font-extrabold mb-1 z-10">{{ $venues->whereNotNull('department_id')->count() }}</div>
        ...
        <div class="bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl shadow-lg p-5 flex flex-col items-center justify-center text-white relative overflow-hidden">
            ...
            <div class="text-3xl font-extrabold mb-1 z-10">{{ $venues->whereNull('department_id')->count() }}</div>
    ...
    @forelse($venues as $venue)
        <div class="relative bg-white border-l-4 border-green-500 shadow-xl rounded-2xl p-6 flex flex-col justify-between hover:shadow-2xl transition-shadow duration-200">
            <div class="flex items-center gap-3 mb-3">
                ...
                <div>
                    <h3 class="text-xl font-extrabold text-gray-900">{{ $venue->name }}</h3>
                    <p class="text-sm text-gray-600">Radius: {{ $venue->radius }}km</p>
                    <div class="flex gap-2 mt-2">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-blue-50 text-blue-700">
                            {{ $venue->department ? $venue->department->name : 'Global' }}
                        </span>
                    </div>
                </div>
            </div>
            <div class="text-xs text-gray-500 mb-3 font-mono">
                <div>Lat: {{ number_format($venue->latitude, 6) }}</div>
                <div>Lng: {{ number_format($venue->longitude, 6) }}</div>
            </div>
            <div class="flex items-center justify-between mt-4">
                <div class="flex items-center gap-2">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $venue->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                        {{ $venue->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </div>
                <div class="flex gap-2">
                    <button onclick="editVenue('{{ $venue->id }}')" class="bg-gray-200 text-gray-700 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-gray-300 transition">Edit</button>
                    <button onclick="deleteVenue('{{ $venue->id }}')" class="bg-red-100 text-red-700 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-red-200 transition">Delete</button>
                </div>
            </div>
        </div>
    @empty
