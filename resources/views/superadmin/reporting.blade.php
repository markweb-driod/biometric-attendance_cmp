...
@section('page-description', 'Comprehensive analytics and reporting for attendance and system performance')
...
<div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold">{{ $stats['attendance']['today'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">This Week:</span>
                    <span class="font-semibold">{{ $stats['attendance']['this_week'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">This Month:</span>
                    <span class="font-semibold">{{ $stats['attendance']['this_month'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm border-t pt-2 mt-2">
                    <span class="text-gray-700 font-medium">Total:</span>
                    <span class="font-bold text-blue-600">{{ $stats['attendance']['total'] ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500">
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Today:</span>
                    <span class="font-semibold">{{ $stats['sessions']['today'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">This Week:</span>
                    <span class="font-semibold">{{ $stats['sessions']['this_week'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">This Month:</span>
                    <span class="font-semibold">{{ $stats['sessions']['this_month'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm border-t pt-2 mt-2">
                    <span class="text-gray-700 font-medium">Total:</span>
                    <span class="font-bold text-purple-600">{{ $stats['sessions']['total'] ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500">
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold">{{ $stats['users']['students'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Lecturers:</span>
                    <span class="font-semibold">{{ $stats['users']['lecturers'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm border-t pt-2 mt-2">
                    <span class="text-gray-700 font-medium">Total Active:</span>
                    <span class="font-bold text-green-600">{{ $stats['users']['total'] ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-orange-500">
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold">{{ $stats['academic']['departments'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Courses:</span>
                    <span class="font-semibold">{{ $stats['academic']['courses'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Classrooms:</span>
                    <span class="font-semibold">{{ $stats['academic']['classrooms'] ?? 0 }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            <div class="space-y-3">
                @if(isset($topPerformingClasses) && $topPerformingClasses->count() > 0)
                    @foreach($topPerformingClasses->take(10) as $class)
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div class="flex-1">
                            <div class="font-semibold text-sm text-gray-900">{{ $class['class_name'] ?? 'N/A' }}</div>
                            <div class="text-xs text-gray-500">{{ $class['course_name'] ?? 'N/A' }} • {{ $class['lecturer_name'] ?? 'N/A' }}</div>
                        </div>
                        <div class="text-right">
                            <div class="text-sm font-bold text-green-600">{{ $class['attendance_rate'] ?? 0 }}%</div>
                            <div class="text-xs text-gray-500">{{ $class['attendance_count'] ?? 0 }} / {{ $class['session_count'] ?? 0 }}</div>
                        </div>
                    </div>
                    @endforeach
                @else
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @if(isset($departmentStats) && $departmentStats->count() > 0)
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 font-medium text-gray-900">{{ $dept['name'] ?? 'N/A' }}</td>
                            <td class="px-4 py-3 text-gray-600 font-mono text-xs">{{ $dept['code'] ?? 'N/A' }}</td>
                            <td class="px-4 py-3">{{ $dept['students_count'] ?? 0 }}</td>
                            <td class="px-4 py-3">{{ $dept['lecturers_count'] ?? 0 }}</td>
                            <td class="px-4 py-3">{{ $dept['courses_count'] ?? 0 }}</td>
                        </tr>
                        @endforeach
                    @else
                        <tr>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <form action="{{ route('superadmin.reporting.attendance-report') }}" method="POST" target="_blank" class="inline">
                @csrf
            ...
            <form action="{{ route('superadmin.reporting.student-performance-report') }}" method="POST" target="_blank" class="inline">
                @csrf
                <input type="hidden" name="format" value="excel">
            ...
            <form action="{{ route('superadmin.reporting.system-analytics-report') }}" method="POST" target="_blank" class="inline">
                @csrf
                <input type="hidden" name="format" value="excel">
