...
@push('styles')
<link rel="preload" href="{{ asset('css/lecturers-optimized.css') }}" as="style">
<link rel="stylesheet" href="{{ asset('css/lecturers-optimized.css') }}">
@endpush
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="even:bg-gray-50 hover:bg-green-50 transition-colors duration-200 align-middle">
                        ...
                        <td class="px-4 py-3 font-medium text-gray-900">{{ $lecturer->user->full_name ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $lecturer->staff_id }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $lecturer->user->email ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $lecturer->department->name ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $lecturer->title ?? 'N/A' }}</td>
                        <td class="px-4 py-3">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold {{ $lecturer->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500' }}">
                                {{ $lecturer->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1.5">
                                <a href="{{ route('superadmin.lecturers.details', $lecturer->id) }}" class="p-1.5 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors" title="View Details">
                                ...
                                <button onclick="openEditModal({{ $lecturer->id }})" class="p-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                </button>
                                <button onclick="openPasswordModal({{ $lecturer->id }}, '{{ $lecturer->user->full_name ?? $lecturer->staff_id }}')" class="p-1.5 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition-colors" title="Update Password">
                                ...
                                <button onclick="toggleLecturerStatus({{ $lecturer->id }}, {{ $lecturer->is_active ? 'true' : 'false' }})" 
                                    data-lecturer-toggle="{{ $lecturer->id }}"
                                    class="p-1.5 {{ $lecturer->is_active ? 'bg-orange-100 text-orange-700 hover:bg-orange-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }} rounded-lg transition-colors" title="{{ $lecturer->is_active ? 'Disable' : 'Enable' }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $lecturer->is_active ? 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636' : 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }}" />
                                    </svg>
                                </button>
                                <button onclick="openDeleteModal({{ $lecturer->id }}, '{{ $lecturer->user->full_name ?? $lecturer->staff_id }}')" class="p-1.5 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors" title="Delete">
