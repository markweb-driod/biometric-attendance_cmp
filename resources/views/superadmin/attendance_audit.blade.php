...
@section('title', 'Attendance Audit & Live Monitoring')
@section('page-title', 'Attendance Audit & Live Monitoring')
@section('page-description', 'Comprehensive attendance tracking with real-time activity monitoring, charts, and detailed analytics')
...
<div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('superadmin.attendance.audit.export') }}" class="px-4 py-2 bg-orange-600 text-white rounded-lg font-medium hover:bg-orange-700 transition">
                Export CSV
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="kpi-card border-l-4 border-blue-500">
            ...
            <div class="kpi-main-value text-blue-600" id="kpi-total">{{ number_format($stats['overall']['total_attempts'] ?? 0) }}</div>
            <div class="kpi-sub-value">All time records</div>
            <div class="kpi-sub-value mt-2">
                Today: <strong id="kpi-today-attempts">{{ $stats['today']['attempts'] ?? 0 }}</strong> | 
                Week: <strong id="kpi-week-attempts">{{ $stats['this_week']['attempts'] ?? 0 }}</strong>
            </div>
        </div>
        ...
        <div class="kpi-card border-l-4 border-green-500">
            ...
            <div class="kpi-main-value text-green-600" id="kpi-present">{{ number_format($stats['overall']['total_present'] ?? 0) }}</div>
            <div class="kpi-sub-value">
                Success Rate: <strong id="kpi-success-rate">{{ $stats['overall']['total_attempts'] > 0 ? round(($stats['overall']['total_present'] / $stats['overall']['total_attempts']) * 100, 1) : 0 }}%</strong>
            </div>
            <div class="kpi-sub-value mt-2">
                Today: <strong id="kpi-today-present">{{ $stats['today']['present'] ?? 0 }}</strong> | 
                Month: <strong id="kpi-month-present">{{ $stats['this_month']['present'] ?? 0 }}</strong>
            </div>
        </div>
        ...
        <div class="kpi-card border-l-4 border-red-500">
            ...
            <div class="kpi-main-value text-red-600" id="kpi-denied">{{ number_format($stats['overall']['total_denied'] ?? 0) }}</div>
            <div class="kpi-sub-value">
                Denial Rate: <strong id="kpi-denial-rate">{{ $stats['overall']['total_attempts'] > 0 ? round(($stats['overall']['total_denied'] / $stats['overall']['total_attempts']) * 100, 1) : 0 }}%</strong>
            </div>
            <div class="kpi-sub-value mt-2">
                Today: <strong id="kpi-today-denied">{{ $stats['today']['denied'] ?? 0 }}</strong> | 
                This Week: <strong id="kpi-week-denied">{{ $stats['this_week']['denied'] ?? 0 }}</strong>
            </div>
        </div>
        ...
        <div class="kpi-card border-l-4 border-purple-500">
            ...
            <div class="kpi-main-value text-purple-600" id="kpi-active-sessions">{{ $stats['sessions']['active'] ?? 0 }}</div>
            <div class="kpi-sub-value">
                Total Sessions: <strong id="kpi-total-sessions">{{ $stats['sessions']['total'] ?? 0 }}</strong>
            </div>
            <div class="kpi-sub-value mt-2">
                Created Today: <strong id="kpi-sessions-today">{{ $stats['today']['sessions'] ?? 0 }}</strong> | 
                Completed: <strong id="kpi-sessions-completed">{{ $stats['sessions']['completed_today'] ?? 0 }}</strong>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="kpi-card border-l-4 border-indigo-500">
            ...
            <div class="text-2xl font-bold text-indigo-600" id="kpi-unique">{{ $stats['overall']['unique_students'] ?? 0 }}</div>
        ...
        <div class="kpi-card border-l-4 border-orange-500">
            ...
            <div class="text-2xl font-bold text-orange-600" id="kpi-spoofers">{{ $stats['overall']['suspected_spoofers'] ?? 0 }}</div>
        ...
        <div class="kpi-card border-l-4 border-teal-500">
            ...
            <div class="text-2xl font-bold text-teal-600" id="kpi-biometric-rate">{{ $stats['biometric']['biometric_success_rate'] ?? 0 }}%</div>
            <div class="text-xs text-gray-500 mt-1">
                Biometric: {{ $stats['biometric']['total_biometric'] ?? 0 }} | Manual: {{ $stats['biometric']['total_manual'] ?? 0 }}
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="attendance-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="table-body">
                    ...
                    @endphp
                    <tr @if($spoofing) class="bg-yellow-50" @endif data-status="{{ $attendance->status }}" data-search="{{ strtolower(($attendance->student->user->full_name ?? '') . ' ' . ($attendance->student->matric_number ?? '') . ' ' . ($attendance->classroom->class_name ?? ''))) }}">
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="font-medium text-gray-900">{{ $attendance->student->user->full_name ?? '-' }}</div>
                            <div class="text-sm text-gray-500">{{ $attendance->student->matric_number ?? '' }}</div>
                            @if($spoofing)
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $attendance->classroom->class_name ?? '-' }}</div>
                            <div class="text-xs text-gray-500">{{ $attendance->classroom->course->course_code ?? '' }}</div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                            {{ $attendance->classroom->course->course_name ?? '-' }}
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                            {{ $attendance->attendanceSession->code ?? '-' }}
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $attendance->captured_at ? \Carbon\Carbon::parse($attendance->captured_at)->format('Y-m-d') : '-' }}</div>
                            <div class="text-xs text-gray-500">{{ $attendance->captured_at ? \Carbon\Carbon::parse($attendance->captured_at)->format('H:i:s') : '-' }}</div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
...
function refreshActivity() {
    fetch('{{ route('superadmin.attendance.audit.live') }}?limit=20')
        .then(res => res.json())
        .then(data => {
            if (data.success) {
