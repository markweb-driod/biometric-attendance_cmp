...
@section('title', 'Audit Logs & Compliance')
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div>
            ...
            <select x-model="filters.resource_type" @change="applyFilters()" 
                ...
                @foreach($filterOptions['resource_types'] as $type)
        ...
        <div>
            ...
            <select x-model="filters.user_type" @change="applyFilters()" 
                ...
                @foreach($filterOptions['user_types'] as $type)
                    <option value="{{ $type }}">{{ ucfirst($type) }}</option>
                @endforeach
            </select>
        </div>
...
function auditLogsApp() {
    return {
        ...
        applyFilters() {
            ...
            window.location.href = '{{ route("hod.audit.index") }}?' + params.toString();
        },
        ...
        async viewDetails(logId) {
            ...
            try {
                const response = await fetch(`{{ route("hod.audit.api.logs.details", ":id") }}`.replace(':id', logId));
        ...
        async generateComplianceReport() {
            ...
            try {
                ...
                const response = await fetch('{{ route("hod.audit.api.compliance-report") }}?' + params.toString());
        ...
        async exportData() {
            ...
            try {
                ...
                const response = await fetch('{{ route("hod.audit.api.export") }}?' + params.toString());
