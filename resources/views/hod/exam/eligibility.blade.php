...
@section('header', 'Exam Eligibility Management')
...
@if(session('execute_waiver'))
<script>
    // Auto-execute waiver after 2FA verification
    document.addEventListener('DOMContentLoaded', function() {
        fetch('{{ route("hod.exam.api.eligibility.execute-waiver") }}', {
            method: 'POST',
            headers: {
            ...
            <div x-show="!eligibilityData || eligibilityData.length === 0" class="p-12 text-center">
                <div class="max-w-md mx-auto">
                    ...
                    <div class="mt-6">
                        ...
                        <div class="mt-6">
                            <a href="{{ route('hod.exam.eligibility.configuration') }}" 
...
function examEligibilityApp() {
    return {
        ...
        eligibilityData: @json($eligibilityData),
        stats: @json($eligibilityStats),
        atRiskStudents: @json($atRiskStudents),
        ...
        applyFilters() {
            ...
            window.location.href = '{{ route("hod.exam.eligibility") }}?' + params.toString();
        },
        ...
        async submitWaiver() {
            ...
            try {
                const response = await fetch('{{ route("hod.exam.api.eligibility.prepare-waiver") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        async submitOverride() {
            ...
            try {
                const response = await fetch('{{ route("hod.exam.api.eligibility.override") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        async calculateEligibility() {
            ...
            try {
                const response = await fetch('{{ route("hod.exam.api.eligibility.calculate") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                if (result.success) {
                    ...
                    if (result.requires_2fa) {
                        window.location.href = result.redirect || '{{ route("hod.two-factor.show") }}';
                    } else {
        ...
        async exportData() {
            ...
            try {
                ...
                const response = await fetch('{{ route("hod.exam.api.eligibility.export") }}?' + params.toString());
