        ...
        <div class="flex justify-between items-center h-16 min-w-0 gap-2">
            ...
            <div class="flex items-center flex-shrink-0 min-w-0">
                ...
                <div class="hidden md:block min-w-0 max-w-[200px] sm:max-w-xs">
                    ...
                    @endif
                    <h1 class="text-lg font-semibold text-gray-900 truncate leading-tight">{{ $pageTitle }}</h1>
                    <p class="text-xs text-gray-500 truncate mt-0.5">Welcome back, {{ $hod->user->full_name ?? 'HOD' }}</p>
                </div>
            </div>
                    ...
                    <input type="text" 
                           ...
                           @input.debounce.300ms="
                               if (searchQuery.length > 2) {
                                   loading = true;
                                   fetch('{{ route('hod.search') }}?q=' + encodeURIComponent(searchQuery))
            ...
            <div class="flex items-center space-x-1 sm:space-x-2 md:space-x-4 flex-shrink-0">
                ...
                <div class="hidden lg:flex items-center space-x-2">
                    <a href="{{ route('hod.monitoring.students') }}" 
                    ...
                    <a href="{{ route('hod.exam.eligibility') }}" 
                       class="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-md transition"
                ...
                <div class="relative" 
                     x-data="{
                         ...
                         unreadCount: 0,
                         loading: false,
                         notificationsUrl: '{{ route('hod.api.notifications') }}',
                         readUrl: '{{ route('hod.api.notifications.read') }}',
                         readAllUrl: '{{ route('hod.api.notifications.read-all') }}',
                         loadNotifications() {
                             this.loading = true;
                    ...
                    <div x-show="open" 
                        ...
                        <div class="p-4">
                            ...
                            <div class="mt-4 pt-3 border-t border-gray-200" x-show="!loading && notifications.length > 0">
                                <a href="{{ route('hod.audit.index') }}" class="text-sm text-green-600 hover:text-green-500 font-medium block text-center">View all notifications</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" 
                        ...
                        <div class="w-7 h-7 sm:w-8 sm:h-8 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0">
                            <span class="text-white text-xs sm:text-sm font-semibold">{{ substr($hod->user->full_name ?? 'HOD', 0, 1) }}</span>
                        </div>
                        <span class="ml-1 sm:ml-2 text-gray-700 font-medium hidden lg:block truncate max-w-[120px]">{{ $hod->user->full_name ?? 'HOD' }}</span>
                    ...
                    <div x-show="open" 
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2 border-b border-gray-100">
                                <p class="text-sm font-medium text-gray-900">{{ $hod->user->full_name ?? 'HOD' }}</p>
                                <p class="text-sm text-gray-500">{{ $hod->user->email ?? 'hod@nsuk.edu.ng' }}</p>
                            </div>
                            
                            <a href="{{ route('hod.profile') }}" 
