...
<div class="min-h-screen bg-gray-50">
        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                <li class="inline-flex items-center">
                    <a href="{{ route('hod.dashboard') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('hod.management.students.index') }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">Student Management</a>
                    </div>
                </li>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8 mb-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div class="flex items-center space-x-6">
                <div class="relative">
                    @if($student->reference_image_path)
                        <img src="{{ asset('storage/' . $student->reference_image_path) }}" 
                    ...
                    @else
                        <div class="h-20 w-20 sm:h-28 sm:w-28 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center shadow-lg border-4 border-blue-200">
                            <span class="text-xl sm:text-2xl font-bold text-white">{{ substr($student->user->full_name, 0, 2) }}</span>
                        </div>
                    @endif
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('hod.management.students.index') }}" class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-medium">{{ $student->user->full_name }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-medium text-gray-500">Matric Number</span>
                    <span class="text-sm font-mono text-gray-900 font-medium">{{ $student->matric_number }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-medium">{{ $student->classrooms->count() }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                ...
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-medium">{{ $attendanceStats->total_records ?? 0 }}</span>
                </div>
                @endif
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 mb-6">
        ...
        <div class="bg-gray-50 rounded-lg p-6 flex flex-col items-center">
            <div class="relative mb-4">
                <img src="{{ asset('storage/' . $student->reference_image_path) }}" 
                     alt="{{ $student->user->full_name }} - Registered Image" 
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
                ...
                <p class="text-2xl font-bold text-blue-900">{{ $attendanceStats->total_records ?? 0 }}</p>
            </div>
            <div class="bg-green-50 p-4 rounded-lg border-l-4 border-green-500">
                <p class="text-sm text-green-600 font-medium mb-1">Present</p>
                <p class="text-2xl font-bold text-green-900">{{ $attendanceStats->present_count ?? 0 }}</p>
            </div>
            <div class="bg-red-50 p-4 rounded-lg border-l-4 border-red-500">
                <p class="text-sm text-red-600 font-medium mb-1">Absent</p>
                <p class="text-2xl font-bold text-red-900">{{ $attendanceStats->absent_count ?? 0 }}</p>
            </div>
            <div class="bg-yellow-50 p-4 rounded-lg border-l-4 border-yellow-500">
                <p class="text-sm text-yellow-600 font-medium mb-1">Late</p>
                <p class="text-2xl font-bold text-yellow-900">{{ $attendanceStats->late_count ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-sm font-semibold text-gray-900">{{ number_format($attendanceRate, 1) }}%</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2.5">
                <div class="bg-blue-600 h-2.5 rounded-full" style="width: {{ min(100, $attendanceRate) }}%"></div>
            </div>
        </div>
        @endif
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="font-semibold text-gray-900 text-sm sm:text-base">{{ $classroom->course->course_name ?? 'Unknown Course' }}</h3>
                ...
                <div class="space-y-2 text-sm text-gray-600">
                    <div class="flex justify-between">
                        <span>Class:</span>
                        <span class="font-medium">{{ $classroom->class_name }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Lecturer:</span>
                        <span class="font-medium">{{ $classroom->lecturer->user->full_name ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>PIN:</span>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
        <div class="flex flex-wrap gap-3 justify-center">
            <button onclick="openEditModal({{ $student->id }})" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                </svg>
                Edit Student
            </button>
            <a href="{{ route('hod.monitoring.students') }}?student={{ $student->id }}" class="inline-flex items-center px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                </svg>
                View Attendance Report
            </a>
            <button onclick="confirmDelete({{ $student->id }})" class="inline-flex items-center px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('hod.management.students.index') }}" class="flex-1 text-center bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition">Go to Management</a>
    ...
    if (confirm('Are you sure you want to delete this student? This action cannot be undone.')) {
        ...
        .then(data => {
            if (data.success) {
                ...
                window.location.href = '{{ route("hod.management.students.index") }}';
            } else {
