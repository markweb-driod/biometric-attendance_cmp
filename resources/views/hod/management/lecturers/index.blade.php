...
function loadStatistics() {
    fetch('{{ route("hod.management.lecturers.api.statistics") }}')
        .then(r => r.json())
        .then(d => {
            if (d.success) {
...
function loadLecturers() {
    ...
    fetch(`{{ route("hod.management.lecturers.api.list") }}?${params}`)
        .then(r => r.json())
        .then(d => {
            if (d.success) {
...
function handleUpload(e) {
    ...
    fetch('{{ route("hod.management.lecturers.api.upload") }}', {
        method: 'POST',
...
function downloadTemplate() {
    window.location.href = '{{ route("hod.management.lecturers.api.template") }}';
}
...
function viewLecturer(id) {
    ...
    fetch(`{{ route("hod.management.lecturers.api.show", ":id") }}`.replace(':id', id))
...
function goToAdvancedDetails() {
    if (currentLecturerId) {
        window.location.href = `{{ route("hod.management.lecturers.show", ":id") }}`.replace(':id', currentLecturerId);
    }
}
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    fetch(`{{ route("hod.management.lecturers.api.update", ":id") }}`.replace(':id', id), {
        method: 'PUT',
        headers: {
