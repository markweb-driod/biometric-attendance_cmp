...
@section('title', 'Lecturer Details - ' . $lecturer->user->full_name)
...
<div class="min-h-screen bg-gray-50">
        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                <li class="inline-flex items-center">
                    <a href="{{ route('hod.dashboard') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-purple-600">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('hod.management.lecturers.index') }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-purple-600 md:ml-2">Lecturer Management</a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">{{ $lecturer->user->full_name }}</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8 mb-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div class="flex items-center space-x-6">
                <div class="relative">
                    <div class="h-20 w-20 sm:h-28 sm:w-28 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center shadow-lg border-4 border-purple-200">
                        <span class="text-xl sm:text-2xl font-bold text-white">{{ substr($lecturer->user->full_name, 0, 2) }}</span>
                    </div>
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $lecturer->user->full_name }}</h1>
                    <p class="text-lg sm:text-xl text-gray-600 font-mono">{{ $lecturer->staff_id }}</p>
                    @if($lecturer->title)
                        <p class="text-base sm:text-lg text-gray-500 mt-1">{{ $lecturer->title }}</p>
                    @endif
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('hod.management.lecturers.index') }}" class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-medium">{{ $lecturer->user->full_name }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-medium text-gray-500">Staff ID</span>
                    <span class="text-sm font-mono text-gray-900 font-medium">{{ $lecturer->staff_id }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-medium text-gray-500">Email Address</span>
                    <span class="text-sm text-gray-900">{{ $lecturer->user->email }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                ...
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900">{{ $lecturer->department->name ?? 'Not assigned' }}</span>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm text-gray-900 font-medium">{{ $lecturer->title ?? 'Not assigned' }}</span>
                </div>
                @if($teachingStats)
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-medium text-gray-500">Active Classes</span>
                    <span class="text-sm text-gray-900 font-medium">{{ $teachingStats->total_classes ?? 0 }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-medium text-gray-500">Total Students</span>
                    <span class="text-sm text-gray-900 font-medium">{{ $teachingStats->total_students ?? 0 }}</span>
                </div>
                @endif
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-medium text-gray-500">Registration Date</span>
                    <span class="text-sm text-gray-900">{{ $lecturer->created_at->format('M d, Y') }}</span>
                </div>
                <div class="flex justify-between items-center py-3">
                    <span class="text-sm font-medium text-gray-500">Last Updated</span>
                    <span class="text-sm text-gray-900">{{ $lecturer->updated_at->format('M d, Y') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Teaching Statistics -->
    @if($teachingStats && $teachingStats->total_classes > 0)
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
            <div class="bg-purple-50 p-4 rounded-lg border-l-4 border-purple-500">
                ...
                <p class="text-2xl font-bold text-purple-900">{{ $teachingStats->total_classes ?? 0 }}</p>
            </div>
            <div class="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
                <p class="text-sm text-blue-600 font-medium mb-1">Total Students</p>
                <p class="text-2xl font-bold text-blue-900">{{ $teachingStats->total_students ?? 0 }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 mb-6">
        ...
        @if($lecturer->classrooms->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($lecturer->classrooms as $classroom)
            <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                ...
                <div class="space-y-2 text-sm text-gray-600">
                    ...
                    <div class="flex justify-between">
                        <span>Students:</span>
                        <span class="font-medium">{{ $classroom->students->count() }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Status:</span>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
        <div class="flex flex-wrap gap-3 justify-center">
            <button onclick="openEditModal({{ $lecturer->id }})" class="inline-flex items-center px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                </svg>
                Edit Lecturer
            </button>
            <a href="{{ route('hod.monitoring.courses') }}?lecturer={{ $lecturer->id }}" class="inline-flex items-center px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                </svg>
                View Teaching Report
            </a>
            <button onclick="confirmDelete({{ $lecturer->id }})" class="inline-flex items-center px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('hod.management.lecturers.index') }}" class="flex-1 text-center bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 transition">Go to Management</a>
...
function confirmDelete(id) {
    if (confirm('Are you sure you want to delete this lecturer? This action cannot be undone.')) {
        fetch(`{{ route("hod.management.lecturers.api.delete", ":id") }}`.replace(':id', id), {
            method: 'DELETE',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                window.location.href = '{{ route("hod.management.lecturers.index") }}';
            } else {
