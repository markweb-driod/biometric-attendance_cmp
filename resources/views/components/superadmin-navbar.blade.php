        ...
        <div class="flex justify-between items-center h-16 min-w-0 gap-2">
            ...
            <div class="flex items-center flex-shrink-0 min-w-0">
                ...
                <div class="hidden md:block min-w-0 max-w-[220px] sm:max-w-xs ml-2">
                    <h1 class="text-lg font-semibold text-gray-900 truncate leading-tight">@yield('page-title', 'Superadmin Dashboard')</h1>
                    <p class="text-xs text-gray-500 truncate mt-0.5">Welcome back, {{ auth()->user()->full_name ?? 'Superadmin' }}</p>
                </div>
            </div>
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        ...
                        <div class="w-7 h-7 sm:w-8 sm:h-8 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0">
                            <span class="text-white text-xs sm:text-sm font-semibold">{{ strtoupper(substr(auth()->user()->full_name ?? 'S', 0, 1)) }}</span>
                        </div>
                        <span class="ml-1 sm:ml-2 text-gray-700 font-medium hidden lg:block truncate max-w-[140px]">{{ auth()->user()->full_name ?? 'Superadmin' }}</span>
                    ...
                    <div x-show="open"
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2 border-b border-gray-100">
                                <p class="text-sm font-medium text-gray-900">{{ auth()->user()->full_name ?? 'Superadmin' }}</p>
                                <p class="text-sm text-gray-500">{{ auth()->user()->email ?? 'superadmin@nsuk.edu.ng' }}</p>
                            </div>
