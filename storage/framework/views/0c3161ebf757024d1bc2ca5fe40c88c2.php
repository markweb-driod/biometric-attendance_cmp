<?php $__env->startSection('title', 'Live Attendance'); ?>
<?php $__env->startSection('page-title', 'Live Attendance'); ?>
<?php $__env->startSection('page-description', 'Mark and monitor attendance for your class in real time'); ?>
...
<div class="max-w-7xl w-full mx-auto px-2 sm:px-6 lg:px-8 py-6" data-session-id="<?php echo e($session->id); ?>">
    <?php if(session('quick_url')): ?>
    <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl flex flex-col sm:flex-row items-center gap-3">
        <div class="flex-1 text-green-900 text-sm">
            ...
            <input type="text" id="quick-attendance-url" class="w-full sm:w-auto bg-green-100 border border-green-300 rounded px-2 py-1 font-mono text-green-800 text-xs" value="<?php echo e(session('quick_url')); ?>" readonly>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-3 mb-2">
          <h2 class="text-2xl font-bold text-gray-900 truncate"><?php echo e($class->code); ?> - <?php echo e($class->name); ?> <span class="text-gray-400 font-normal">(<?php echo e($class->level); ?>)</span></h2>
        </div>
        <div class="text-sm text-gray-600 mb-2">
          <span class="font-semibold">Lecturer:</span> <?php echo e($lecturer->name); ?> | 
          <span class="font-semibold">Course:</span> <?php echo e($class->course->name ?? 'N/A'); ?> | 
          <span class="font-semibold">PIN:</span> <?php echo e($class->pin); ?>

        </div>
        <div class="flex items-center gap-3 mb-2">
          <span class="text-sm text-gray-500">Attendance Code:</span>
          <span class="font-mono text-lg bg-green-100 border border-green-300 rounded px-3 py-1 text-green-800 tracking-widest select-all" id="session-code"><?php echo e($session->code); ?></span>
          <button onclick="navigator.clipboard.writeText('<?php echo e($session->code); ?>'); this.textContent='Copied!'; setTimeout(()=>this.textContent='Copy',1200);" class="ml-2 px-3 py-1 bg-green-600 text-white rounded text-xs font-semibold hover:bg-green-700 transition">Copy</button>
        </div>
        <div class="flex items-center gap-3 mb-2">
            ...
            <div class="flex items-center bg-gray-50 border border-gray-300 rounded px-2 py-1 max-w-md w-full">
                <input type="text" readonly value="<?php echo e(route('student.attendance-capture', ['code' => $session->code])); ?>" class="bg-transparent border-none text-xs text-gray-600 flex-1 focus:ring-0 select-all" id="session-link-input">
        ...
        <div class="text-xs text-gray-400 mb-2">
          <span class="font-semibold">Session started:</span> <?php echo e($session->start_time->format('Y-m-d H:i')); ?> | 
          <span class="font-semibold">Duration:</span> <?php echo e($session->start_time->diffForHumans()); ?> | 
          <span class="font-semibold">Status:</span> <span class="text-green-600 font-semibold"><?php echo e(ucfirst($session->status)); ?></span>
        </div>
        <div class="mt-2 flex items-center gap-2">
          <input type="number" id="recalibrate-radius" class="border rounded px-2 py-1 w-24" placeholder="Radius (m)" value="<?php echo e($session->radius ?? 50); ?>">
      ...
      <div class="flex flex-col gap-3 min-w-[260px]">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div class="bg-blue-50 rounded-lg px-4 py-2 text-center">
            ...
            <div class="text-xl font-bold text-blue-900" id="kpi-total"><?php echo e($class->students->count()); ?></div>
          </div>
          <div class="bg-green-50 rounded-lg px-4 py-2 text-center">
            <div class="text-xs text-green-700 font-semibold">Present</div>
            <div class="text-xl font-bold text-green-900" id="kpi-present"><?php echo e($attendances->where('status', 'present')->count()); ?></div>
          </div>
          <div class="bg-red-50 rounded-lg px-4 py-2 text-center">
            <div class="text-xs text-red-700 font-semibold">Absent</div>
            <div class="text-xl font-bold text-red-900" id="kpi-absent"><?php echo e($class->students->count() - $attendances->where('status', 'present')->count()); ?></div>
          </div>
          <div class="bg-purple-50 rounded-lg px-4 py-2 text-center">
            ...
            <div class="text-xl font-bold text-purple-900" id="kpi-percent">
              <?php echo e($class->students->count() > 0 ? round(($attendances->where('status', 'present')->count() / $class->students->count()) * 100) : 0); ?>%
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
      ...
      <form method="POST" action="<?php echo e(route('lecturer.attendance.mark', ['sessionId' => $session->id])); ?>">
        <?php echo csrf_field(); ?>
      ...
      <form method="POST" action="<?php echo e(route('lecturer.attendance.end', ['sessionId' => $session->id])); ?>" class="mt-8">
        <?php echo csrf_field(); ?>
...
function sessionId() {
  ...
  return container ? container.dataset.sessionId : '<?php echo e($session->id); ?>';
}
...
function markAttendance(studentId, action) {
  ...
  fetch(`/lecturer/attendance/session/<?php echo e($session->id); ?>/mark`, {
    method: 'POST',
    headers: {
...
document.getElementById('recalibrate-btn').onclick = function() {
    ...
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(function(position) {
            fetch('/lecturer/attendance/recalibrate/' + <?php echo e($session->id); ?>, {
                method: 'POST',
                headers: {
...
<?php $__env->stopSection(); ?> 
<?php echo $__env->make('layouts.lecturer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\biometric-attendance\resources\views\lecturer\attendance_session.blade.php ENDPATH**/ ?>