<?php $__env->startSection('title', 'Course Assignment Management'); ?>
...
<style>
    ...
    .course-checkbox:checked + .course-item {
    ...
    <div class="">
        <div class="max-w-7xl mx-auto py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div class="flex-1">
                    <h1 class="text-3xl sm:text-4xl font-bold text-white mb-2" style="font-family: 'Montserrat', sans-serif;">
                        Course Assignment Management
                    </h1>
                    <p class="text-green-50 text-sm sm:text-base font-medium">
                        Assign courses to lecturers and manage course assignments efficiently
                    </p>
                </div>
                <div class="flex gap-3">
                    <button onclick="openCreateCourseModal()" 
                            class="inline-flex items-center justify-center px-5 py-2.5 bg-white text-green-700 rounded-lg shadow-md hover:bg-green-50 hover:shadow-lg transition-all font-semibold text-sm">
                    ...
                    <button onclick="refreshData()" 
                            class="inline-flex items-center justify-center px-5 py-2.5 bg-white/20 backdrop-blur-sm text-white rounded-lg border border-white/30 hover:bg-white/30 transition-all font-semibold text-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="courseDetailsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50 backdrop-blur-sm">
    <div class="relative top-10 mx-auto p-0 border-0 w-full max-w-2xl shadow-2xl rounded-2xl bg-white my-8">
        <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 rounded-t-2xl">
            <div class="flex justify-between items-center">
...
const baseUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.index") : route("superadmin.course-assignment.index")); ?>';
const apiUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.lecturers") : route("superadmin.course-assignment.api.lecturers")); ?>';
const coursesApiUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.courses") : route("superadmin.course-assignment.api.courses")); ?>';
const assignUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.assign") : route("superadmin.course-assignment.api.assign")); ?>';
const unassignUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.unassign") : route("superadmin.course-assignment.api.unassign")); ?>';
const createCourseUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.create-course") : route("superadmin.course-assignment.api.create-course")); ?>';
const dropdownDataUrl = '<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.dropdown-data") : route("superadmin.course-assignment.api.dropdown-data")); ?>';
                        ...
                        tbody.innerHTML = assignedLecturers.map(lecturer => `
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="selectLecturer(${lecturer.id}, '${lecturer.name.replace(/'/g, "\\'")}'); viewLecturerCourses();" 
...
function viewLecturerCourses() {
    ...
    const lecturerCoursesUrl = `<?php echo e(auth("hod")->check() ? route("hod.management.course-assignment.api.lecturer.courses", ["lecturerId" => ":id"]) : route("superadmin.course-assignment.api.lecturer.courses", ["lecturerId" => ":id"])); ?>`.replace(':id', selectedLecturerId);
...
<?php echo $__env->make('hod.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\biometric-attendance\resources\views\hod\management\course-assignment\index.blade.php ENDPATH**/ ?>