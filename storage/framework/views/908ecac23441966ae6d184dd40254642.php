...
function loadStatistics() {
    fetch('<?php echo e(route("hod.management.students.api.statistics")); ?>')
...
function loadStudents() {
    ...
    fetch(`<?php echo e(route("hod.management.students.api.list")); ?>?${params}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function handleUpload(e) {
    ...
    fetch('<?php echo e(route("hod.management.students.api.upload")); ?>', {
        method: 'POST',
        headers: {
...
function downloadTemplate() {
    window.location.href = '<?php echo e(route("hod.management.students.api.template")); ?>';
}
...
function viewStudent(id) {
    currentStudentId = id;
    fetch(`<?php echo e(route("hod.management.students.api.show", ":id")); ?>`.replace(':id', id))
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function goToAdvancedDetails() {
    if (currentStudentId) {
        window.location.href = `<?php echo e(route("hod.management.students.show", ":id")); ?>`.replace(':id', currentStudentId);
    }
}
...
function handleUpdate(e) {
    ...
    fetch(`<?php echo e(route("hod.management.students.api.update", ":id")); ?>`.replace(':id', id), {
        method: 'PUT',
        headers: {
...
<?php echo $__env->make('hod.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\biometric-attendance\resources\views\hod\management\students\index.blade.php ENDPATH**/ ?>