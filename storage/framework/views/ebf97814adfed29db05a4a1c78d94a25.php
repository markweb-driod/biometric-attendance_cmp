  ...
  <div class="w-full max-w-xs sm:max-w-sm mx-auto p-0 sm:p-4 mt-4">
    ...
    <div class="bg-gradient-to-br from-white via-green-50 to-green-100 rounded-3xl shadow-2xl border-t-4 border-green-400 px-3 py-4 sm:px-5 sm:py-6 flex flex-col items-center w-full" style="min-height:unset;height:auto;">
      ...
      <div class="flex items-center justify-between w-full mb-4">
        <h2 class="text-l font-extrabold text-green-800 tracking-tight text-center flex-1">Student Attendance System</h2>
...
<div id="details-modal" class="fixed inset-0 bg-black bg-opacity-40 flex items-start justify-center z-50 hidden transition-all">
  <div class="bg-white rounded-3xl shadow-2xl border border-green-200 w-full max-w-full sm:max-w-md lg:max-w-lg xl:max-w-xl p-2 sm:p-6 lg:p-8 animate-fade-in mx-2 my-12 flex flex-col overflow-y-auto max-h-[90vh]">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <button id="close-modal" type="button" class="text-gray-400 hover:text-gray-600 transition-colors p-1">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('back-to-validate').addEventListener('click', function() {
        detailsModal.classList.add('hidden');
        // Reset form
        fetchForm.reset();
        errorMessage.classList.add('hidden');
        // Focus on matric input
        matricInput.focus();
    });
    ...
    fetchForm.addEventListener('submit', function(e) {
      ...
      fetch('<?php echo e(url('api/student/fetch-details')); ?>', {
            method: 'POST',
            headers: {
        ...
        .then(function(result) {
            ...
            '<div><span class="font-bold">Class:</span> ' + result.data.classroom.name + ' (' + result.data.classroom.code + ')</div>';
                ...
                if (result.data && result.data.requires_face_registration) {
                    showToast('Face registration required. Redirecting to registration page...', 'error');
                    setTimeout(() => {
                        window.location.href = result.data.redirect_to || '/student/register-face';
                    }, 2000);
                } else {
    ...
    proceedCaptureBtn.addEventListener('click', function() {
      if (!capturedImage) {
        ...
        var matric = matricInput.value.trim();
        var code = codeInput.value.trim();
      fetch('<?php echo e(url('api/student/capture-attendance')); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php $__env->stopPush(); ?> 
<?php echo $__env->make('layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\biometric-attendance\resources\views\student\attendance_capture_backup.blade.php ENDPATH**/ ?>