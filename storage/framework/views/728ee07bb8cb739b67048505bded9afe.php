...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Lecturer Portal'); ?> - NSUK Biometric Attendance</title>
...
<body class="bg-gray-50">
    ...
    <?php if(session('success')): ?>
        <script>window.addEventListener('DOMContentLoaded',function(){window.dispatchEvent(new CustomEvent('toast',{detail:{message:<?php echo json_encode(session('success'), 15, 512) ?>,type:'success'}}));});</script>
    <?php endif; ?>
    <?php if(session('error')): ?>
        <script>window.addEventListener('DOMContentLoaded',function(){window.dispatchEvent(new CustomEvent('toast',{detail:{message:<?php echo json_encode(session('error'), 15, 512) ?>,type:'error'}}));});</script>
    <?php endif; ?>
    <?php if(session('info')): ?>
        <script>window.addEventListener('DOMContentLoaded',function(){window.dispatchEvent(new CustomEvent('toast',{detail:{message:<?php echo json_encode(session('info'), 15, 512) ?>,type:'info'}}));});</script>
    <?php endif; ?>
    <!-- Sidebar -->
    <div id="sidebar" class="fixed inset-y-0 left-0 z-50 w-60 bg-green-600 shadow-lg transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col">
        ...
        <nav class="flex-1 overflow-y-auto px-2 py-4 space-y-1">
            ...
            <!-- My Courses -->
            <a href="<?php echo e(route('lecturer.courses.index')); ?>" class="nav-link flex items-center px-3 py-2 text-base font-medium text-white rounded-lg hover:bg-green-700 hover:text-white transition group <?php echo e(request()->routeIs('lecturer.courses*') ? 'bg-green-800' : ''); ?>">
                <svg class="w-5 h-5 mr-3 text-white group-hover:text-white transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Grading -->
            <a href="<?php echo e(route('lecturer.grading.index')); ?>" class="nav-link flex items-center px-3 py-2 text-base font-medium text-white rounded-lg hover:bg-green-700 hover:text-white transition group <?php echo e(request()->routeIs('lecturer.grading*') ? 'bg-green-800' : ''); ?>">
                <svg class="w-5 h-5 mr-3 text-white group-hover:text-white transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Venues -->
            <a href="<?php echo e(route('lecturer.venues.index')); ?>" class="nav-link flex items-center px-3 py-2 text-base font-medium text-white rounded-lg hover:bg-green-700 hover:text-white transition group <?php echo e(request()->routeIs('lecturer.venues*') ? 'bg-green-800' : ''); ?>">
                <svg class="w-5 h-5 mr-3 text-white group-hover:text-white transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4 border-t border-green-700 flex-shrink-0 bg-green-600">
            <div class="flex items-center space-x-2">
                ...
                <form method="POST" action="<?php echo e(route('lecturer.logout')); ?>">
                    <?php echo csrf_field(); ?>
    ...
    <div class="lg:pl-60 min-h-screen flex flex-col">
            ...
            <div class="flex items-center justify-between h-20 px-3 sm:px-4 lg:px-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-2 lg:ml-0">
                        <h1 class="text-base sm:text-lg lg:text-xl font-bold text-gray-800 leading-tight"><?php echo $__env->yieldContent('page-title', 'Dashboard'); ?></h1>
        ...
        <footer class="w-full bg-white border-t border-gray-200 py-4 px-4 text-center text-xs text-gray-500 mt-auto">
            <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
                <span>&copy; <?php echo e(date('Y')); ?> Computer Science Department, Nasarawa State University, Keffi. All rights reserved.</span>
...
</html> <?php /**PATH C:\xampp\htdocs\biometric-attendance\resources\views\layouts\lecturer.blade.php ENDPATH**/ ?>