...
<?php $__env->startSection('page-description', 'Manage all classes, assign lecturers, and set levels'); ?>
...
<div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" id="stats-cards">
        <div class="bg-green-50 rounded-xl shadow p-4 flex flex-col items-center">
            ...
            <div class="text-xl font-bold text-gray-900" id="kpi-classes"><?php echo e($stats['total'] ?? '-'); ?></div>
        </div>
        <div class="bg-blue-50 rounded-xl shadow p-4 flex flex-col items-center">
            ...
            <div class="text-xl font-bold text-gray-900" id="kpi-active"><?php echo e($stats['active'] ?? '-'); ?></div>
        </div>
        <div class="bg-purple-50 rounded-xl shadow p-4 flex flex-col items-center">
            ...
            <div class="text-xl font-bold text-gray-900" id="kpi-inactive"><?php echo e($stats['inactive'] ?? '-'); ?></div>
        </div>
        <div class="bg-orange-50 rounded-xl shadow p-4 flex flex-col items-center">
            ...
            <div class="text-xl font-bold text-gray-900" id="kpi-created"><?php echo e($stats['last_created'] ? \Carbon\Carbon::parse($stats['last_created'])->format('Y-m-d H:i') : '-'); ?></div>
        </div>
    </div>
    <!-- Header Section -->
    ...
    <form method="GET" class="flex flex-col md:flex-row gap-2 w-full mb-4">
        <input id="searchInput" type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search by name, code, lecturer..." class="w-full md:w-64 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm">
        <select id="levelFilter" name="level" class="w-full md:w-40 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm">
            ...
            <?php $__currentLoopData = $levels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $level): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($level); ?>" <?php if(request('level') == $level): ?> selected <?php endif; ?>><?php echo e($level); ?> Level</option>
        ...
        <select id="lecturerFilter" name="lecturer" class="w-full md:w-40 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm">
            ...
            <?php $__currentLoopData = $lecturers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lecturer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($lecturer->id); ?>" <?php if(request('lecturer') == $lecturer->id): ?> selected <?php endif; ?>><?php echo e($lecturer->name); ?></option>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody id="classesTable" class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="even:bg-gray-50 hover:bg-green-50 transition-colors duration-200 align-middle">
                        <td class="px-4 py-3 font-medium text-gray-900"><?php echo e($class->course->course_code ?? 'N/A'); ?></td>
                        <td class="px-4 py-3 font-medium text-gray-900"><?php echo e($class->class_name); ?></td>
                        <td class="px-4 py-3 text-gray-700"><?php echo e($class->course->academicLevel->name ?? 'N/A'); ?></td>
                        <td class="px-4 py-3 text-gray-700"><?php echo e($class->lecturer->user->full_name ?? 'Unassigned'); ?></td>
                        <td class="px-4 py-3 text-gray-700"><?php echo e($class->schedule ?? 'Not set'); ?></td>
                        <td class="px-4 py-3 text-gray-700"><?php echo e($class->pin); ?></td>
                        <td class="px-4 py-3">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold <?php echo e($class->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'); ?>">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex flex-row items-center justify-center gap-2">
                                <a href="/superadmin/classes/<?php echo e($class->id); ?>" class="inline-flex items-center px-2.5 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-xs font-semibold hover:bg-gray-200 shadow-sm" title="View">
                                ...
                                <button class="inline-flex items-center px-2.5 py-1.5 bg-blue-500 text-white rounded-lg text-xs font-semibold hover:bg-blue-600 shadow-sm" title="Edit" onclick="openEditModal(<?php echo e($class->id); ?>)">
                                ...
                                <button class="inline-flex items-center px-2.5 py-1.5 bg-red-500 text-white rounded-lg text-xs font-semibold hover:bg-red-600 shadow-sm" title="Delete" onclick="deleteClass(<?php echo e($class->id); ?>)">
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\biometric-attendance\resources\views\superadmin\classes.blade.php ENDPATH**/ ?>